...
let Secimage =  new Image();
Secimage.src= 'download.png';
        ...
        for(let j=1; j<hei-1;j++)
        {
            ...
            for(let x=0; x<3;x++)
            {
                ...
                for(k=0;k<3;k++)
                {
                    ...
                    {
                        let f=(i-1+l)*4+ (j-1+k) * wid*4;
                        avg[h]=imgPrevPixels[f+x];
                        h++;
                    }
                }
                avg.sort();
                imagePixels[i*4+j*wid*4+x]=avg[5];
            }
        }
...
function Combine(imgData)
{
    ...
    Secimage.width= canvas.width;
    Secimage.height= canvas.height;
    canvas2.width=canvas.width;
    canvas2.height=canvas.height;
    ctx2.drawImage(Secimage, 0 , 0,imgData.width,imgData.height);
    
    let sePix = ctx2.getImageData(0, 0, imgData.width, imgData.height).data;
    ...
    for(let i=0;i<wid;i++)
    {
        for(let j=0; j<hei;j++)
        {
            ...
            {
                let f=(i)*4+ (j) * wid*4;
                imagePixels[f+x]=imagePixels[f+x]+sePix[f+x];
            }   
        }
